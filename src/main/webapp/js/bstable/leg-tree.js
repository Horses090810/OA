...
function isShow(that){if($(that).parent().find("ul").is(":visible")){$(that).parent().find("ul").hide();$(that).attr("src",'/img/add.png');}else{if($(that).next().next().next().next().length>0){$(that).attr("src",'img/minus.png');}else{$(that).attr("style",'opacity: 0;');}
...
var ids=0;function createTree(data){var str='<ul>';for(var i=0;i<data.length;i++){ids++;str+='<li><img class="isShow" src="img/minus.png" />'
...
$(tree).html(createTree(data));$.each($("input:checkbox:checked"),function(){checkParent(this)});$.each($("input"),function(){if(this.getAttribute('data-show')=='false'){$(this).parent().find("ul").hide();$(this).prev()[0].setAttribute("src",'/img/add.png');}else{$(this).parent().find("ul").show();if($(this).next().next().next().length>0){$(this).prev()[0].setAttribute("src",'img/minus.png');}else{$(this).prev()[0].setAttribute("style",'opacity: 0;');}}});},})
